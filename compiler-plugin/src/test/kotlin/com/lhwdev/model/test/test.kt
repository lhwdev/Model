...
import com.lhwdev.model.plugin.compiler.ModelComponentRegistrar
...
const val DEST = "D:\\LHW\\develop\\kotlin\\compiler-plugin\\Model\\compiler-plugin\\build\\myOutputTest"
...
class Test {
	...
	fun `test compiler`() {
		...
		K2JVMCompiler().exec(object : MessageCollector {
			...
			internalArguments =
				listOf(LanguageFeatureSetting(LanguageFeature.InlineClasses, LanguageFeature.State.ENABLED))
...
//			""".trimIndent(),
				...
				*getKtFiles("D:\\LHW\\develop\\kotlin\\compiler-plugin\\Model\\core\\src\\commonMain\\kotlin"),
				*getKtFiles("D:\\LHW\\develop\\kotlin\\compiler-plugin\\Model\\core\\src\\jvmMain\\kotlin")
			)
		})
...
//
	...
	private fun getKtFiles(path: String): Array<String> =
		File(path)
			.listKtFilesRecursive()
...
//			.also { it.joinToString { it.toString() }.also { log(it) } }
			.find { resourcesPath ->
				ServiceLoaderLite.findImplementations(ComponentRegistrar::class.java, listOf(resourcesPath.toFile()))
					.any { implementation -> implementation == ModelComponentRegistrar::class.java.name }
			}!!.toString()
...
fun LanguageFeatureSetting(languageFeature: LanguageFeature, state: LanguageFeature.State) =
	ManualLanguageFeatureSetting(
		languageFeature, state, "-XXLanguage:${
			when(state) {
