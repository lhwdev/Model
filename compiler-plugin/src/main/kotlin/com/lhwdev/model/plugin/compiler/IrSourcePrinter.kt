@file:Suppress("NOTHING_TO_INLINE", "EnumEntryName")
@file:OptIn(ObsoleteDescriptorBasedAPI::class)

package com.lhwdev.model.plugin.compiler

import com.lhwdev.model.plugin.compiler.util.IrComponent
import com.lhwdev.model.plugin.compiler.util.IrComponentVisitorVoidWithContext
import com.lhwdev.model.plugin.compiler.util.dumpPreview
import com.lhwdev.model.plugin.compiler.util.pluginContext
...
//		patchDeclarationParentsChildren()
	acceptVoid(IrAllSourcePrinterVisitor(this, context, config, SourceColorConfig.ansi))
}

fun IrElement.dumpSrc(
	context: IrPluginContext = pluginContext,
	config: SourcePrintConfig = SourcePrintConfig.default
) = buildString {
//		patchDeclarationParentsChildren()
	acceptVoid(IrAllSourcePrinterVisitor(this, context, config, SourceColorConfig.disabled))
}
...
//		patchDeclarationParentsChildren()
	acceptVoid(IrDeclarationHeadPrinterVisitor(this, context, config, SourceColorConfig.ansi))
}

fun IrElement.dumpSrcHead(
	context: IrPluginContext = pluginContext,
	config: SourcePrintConfig = SourcePrintConfig.default
) = buildString {
//		patchDeclarationParentsChildren()
	acceptVoid(IrDeclarationHeadPrinterVisitor(this, context, config, SourceColorConfig.disabled))
}
...
fun IrType.dumpColored(
	...
	IrAllSourcePrinterVisitor(this, context, config, SourceColorConfig.ansi).apply {
		print()
	}
}
...
enum class SourceColorConfig { disabled, ansi }
...
private class IrSourcePrinterException(message: String, cause: Throwable) : RuntimeException(message, cause)


private class IrAllSourcePrinterVisitor(
	out: Appendable, context: IrPluginContext, config: SourcePrintConfig, colorConfig: SourceColorConfig
) : IrSourcePrinterVisitor(out, context, config, colorConfig) {
	override val renderContent: Boolean get() = true
	override val renderBody: Boolean get() = true
}

private class IrDeclarationHeadPrinterVisitor(
	out: Appendable, context: IrPluginContext, config: SourcePrintConfig, colorConfig: SourceColorConfig
) : IrSourcePrinterVisitor(out, context, config, colorConfig) {
...
private abstract class IrSourcePrinterVisitor(
	...
	val colorConfig: SourceColorConfig
	...
	fun print(content: CharSequence, type: Type) {
		...
		fun doPrint(headType: SpaceType, tailType: SpaceType) {
			...
			printer.print(
				(if(hasHeadSpace) " " else "") + colorString + content
			)
			lastSpace = tailType
		}
...
//	}
	...
	private fun printReference(content: IrDeclarationWithName, type: Type) {
		val names = mutableListOf(content.name.toString())
		var current: IrDeclaration = content
		var infiniteRecursionGuard = 0
		while(infiniteRecursionGuard < 10) {
			infiniteRecursionGuard++
			val parent = current.parent
			if(parent !is IrPackageFragment && parent !is IrModuleFragment && parent is IrDeclaration) {
		...
		for((index, name) in names.asReversed().withIndex()) {
			if(index != 0) print(".", Type.separator)
			print(name, type)
		}
	...
	private fun IrClassifierSymbol.printDeclaration() {
		when(this) {
			is IrClassSymbol -> print(
				owner.name,
				if(owner.kind == ClassKind.INTERFACE) Type.interfaceClass else Type.classClass
			)
			is IrTypeParameterSymbol -> {
				print(owner.name, Type.typeParameter)
	...
	private fun IrClassifierSymbol.printReference() {
		when(this) {
			is IrClassSymbol -> printReference(
				owner,
				if(owner.kind == ClassKind.INTERFACE) Type.interfaceClass else Type.classClass
			)
			is IrTypeParameterSymbol -> {
				printReference(owner, Type.typeParameter)
				// descriptor.printOrigin()
			}
			else -> error(toString())
		}
	...
	private fun errorLocation(target: IrElement): String = (scopeStack.map { it.irElement } + target).joinToString(separator = "/") {
		val element = it
		val type = when(element) {
			is IrClass -> element.kind.toString().toLowerCase()
			is IrFunction -> "fun"
			is IrFile -> "file ${element.name}"
			else -> null
		}
		when {
			type != null && element is IrDeclarationWithName -> type + " " + element.name.toString()
			type != null -> type
			else -> "$target"
		}
	}
	...
	fun onError(target: IrElement, error: Throwable) {
		if(error is IrSourcePrinterException) return // nested error
		printError("[error: $target]")
		throw IllegalStateException("IrSourcePrinter: unexpected error at ${errorLocation(target)}", error) // DEBUG
	}
	...
	override fun visitClassNew(declaration: IrClass): Unit = declare(declaration) {
		...
		primaryConstructor?.let { it ->
			...
			val declarationsToShow = declarations
				...
				.filter { it.origin != IrDeclarationOrigin.FAKE_OVERRIDE && it.origin != IrDeclarationOrigin.GENERATED_DATA_CLASS_MEMBER && it.origin != IrDeclarationOrigin.DELEGATED_MEMBER && it.origin != IrDeclarationOrigin.DELEGATE }
	...
	override fun visitPropertyNew(declaration: IrProperty) = declare(declaration) {
		...
		if(backingField != null) {
			...
			if(isDelegated) {
				...
				if(initializer == null) printError("no initializer")
				else initializer.expression.print()
...
//		printCallPrefix()
	...
	override fun visitClassReference(expression: IrClassReference) {
		expression.symbol.printReference()
		print("::", Type.importantSeparator)
		printKeyword("class")
...
//		val superQualifiedSymbol = superQualifierSymbol
	...
	override fun visitGetObjectValue(expression: IrGetObjectValue) = expr(expression) {
		printReference(expression.symbol.owner, Type.objectClass)
	...
	override fun visitSetValue(expression: IrSetValue) = expr(expression) {
		printReference(symbol.owner, symbol.type)
		symbol.descriptor.printOrigin()
		print("=", Type.specialOperator)
		value.print()
